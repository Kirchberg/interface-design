...
description: "Use for product interface design work in coding agents: dashboards, admin panels, SaaS apps, tools, settings pages, data interfaces, UI components, visual direction, image-based references, design-system memory, and interaction craft. Do not use for landing pages, marketing sites, campaigns, or brand-only visual exploration."
---
...
5. For greenfield screens, major redesigns, vague visual direction, or post-build craft critique, read `references/imagegen.md` and use image generation as a visual companion when available.
...
Use image generation only when it clarifies a product UI decision. Good uses include direction boards, medium-fidelity UI references, screenshot paintovers, and project-bound raster assets such as empty states or onboarding images.
...
## Invocation Across Install Surfaces

Different tools may expose this skill through slash commands, skill mentions, generated command wrappers, or natural language. If the user asks for these actions through `/interface-design`, `$interface-design`, command wrappers, or plain language, perform the equivalent inline:
